...
一套行为准则，用于改善 AI 编码助手的行为，源自 [Andrej Karpathy 的观察](https://x.com/karpathy/status/2015883857489522876) 关于 LLM 编码陷阱的总结。支持 **Claude Code**、**Cursor**、**OpenCode** 和 **Codex**。
...
交互式脚本，可选择安装到 Claude Code、Cursor、OpenCode 和/或 Codex。
...
OpenCode 也会读取 `CLAUDE.md`，所以上面的选项 B 同样适用。

**选项 B：OpenCode Skill**

将 skill 目录复制到 OpenCode 技能文件夹：
```bash
mkdir -p ~/.config/opencode/skills
cp -r skills/karpathy-guidelines ~/.config/opencode/skills/
```

详见 **[OPENCODE.md](OPENCODE.md)**。
...
Codex 使用 `AGENTS.md` 作为主要指令文件，支持在子目录中层级放置。详见 **[CODEX.md](CODEX.md)**。
...
- **Cursor:** 本仓库包含已提交的 Cursor 项目规则。详见 [`CURSOR.md`](CURSOR.md)。
- **OpenCode:** 本仓库包含 `AGENTS.md` 文件，OpenCode 会自动读取。详见 [`OPENCODE.md`](OPENCODE.md)。
- **Codex:** 本仓库包含 `AGENTS.md` 文件，Codex 会自动读取。详见 [`CODEX.md`](CODEX.md)。
